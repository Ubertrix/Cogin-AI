...
class CodingExpert(BaseExpert):
    """خبير البرمجة المطور: تم تعديله ليدعم التوليد الحر بدلاً من الردود الجاهزة"""
    def __init__(self, input_dim=1024, output_dim=1024):
        super().__init__(name="Coding", input_dim=input_dim, output_dim=output_dim)
        print(f"{self.name} Expert: Generative Mode Active.")

    @shape_guard
    def process(self, x, input_text=""):
        """
        Modified: Removed all hardcoded code templates.
